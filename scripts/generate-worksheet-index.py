...
(function () {{
  ...
  function scrollToResults() {{
    ...
    const target = count || firstCard || document.getElementById('worksheet-catalogue') || document.getElementById('all-worksheet-downloads');
    if (!target) return;
  ...
  function readQueryFromUrl() {{
    ...
    if (sk) skill.value = sk;
    return Boolean(q || y || s || sk);
  }}

  search.addEventListener('input', onSearchInput);
  [year, subject, skill].forEach(control => {{
    control.addEventListener('change', () => applyAndScroll(true));
  }});
